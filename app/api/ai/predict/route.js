...
export async function POST(req) {
    try {
        const { role, userData, inventory, sales, language = "English" } = await req.json();
...
The user is a ${role.toUpperCase()} named ${userData?.name || "Partner"} based in ${userData?.location || "India"}.
${role === 'wholesaler' ? `They sell to retailers. Total Orders: ${userData?.totalSales || 0}.` : `They buy from wholesalers to consumers. Total Orders: ${userData?.totalSales || 0}.`}
