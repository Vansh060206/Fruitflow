...
function RetailerBrowseContent() {
  ...
  const filteredFruits = fruitsData.filter((fruit) => fruit.name.toLowerCase().includes(searchTerm.toLowerCase()))
        ...
        <main className="flex-1 min-h-screen">
          ...
          <header className="bg-white/5 backdrop-blur-xl border-b border-white/10 p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Browse Fresh Fruits</h2>
                  <p className="text-white/60 text-sm">Discover the best quality fruits for your store</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button className="flex items-center gap-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg px-6 py-3 text-white hover:border-purple-500/50 hover:bg-purple-500/10 transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredFruits.map((fruit, index) => (
                ...
                >
                  {/* Stock Badge */}
                  {fruit.inStock && (
                    <div className="absolute top-3 right-3 bg-emerald-500/20 text-emerald-400 text-xs font-bold px-2 py-1 rounded border border-emerald-500/30">
                      In Stock
                    </div>
                  )}
            ...
            {filteredFruits.length === 0 && (
              <div className="text-center py-20">
                <p className="text-white/40 text-lg">No fruits found matching your search.</p>
              </div>
            )}
          </div>
        </main>
